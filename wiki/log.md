...
## 2026-09-19 · 补上本轮记下的三个内容缺口：显著性检验 / 缓存计费 / 三条「孤儿」术语落位

- **类型**：内容 + 工程
- **改了什么**：
  1. **`eval-01` 补「显著性检验」小节（4.1）**：新增 `mcnemar_test.py` 代码块，
     讲清「差 3 个点算不算提升」的判定 —— 配对样本用 McNemar、p 值不是效果大小、
     多次比较会 p-hacking。补上 `compare()` 里「stdev 阈值」这个极简判据没说清的部分。
  2. **`llm-02` 补「缓存计费」小节（3.1）**：新增 `cache_billing.py` 代码块，
     讲清命中 / 未命中的分档计价，以及「压缩上下文可能省了 token、却把前缀从命中打成未命中、
     单价翻数倍」的反直觉权衡。
  3. **三条没被任何正文引用的术语落位**：`repetition-penalty` → `llm-04`（低温复读的对症旋钮）；
     `prefix-fingerprint` → `llm-02`（前缀稳定性检查脚本里点题）；
     `lenient-parsing` → `harness-03`（三层约束的「先解析再校验」）。词库再无孤儿条目。
- **为什么**：这三条正是上一轮 `wiki/tasks.md` 第三节「已知缺口」里记下的 ——
  看板存在的意义就是让「记下没做」的点不会丢，这轮把它消化掉。
- **连带改动**：
  - `scripts/verify-build.mjs` 把「代码块总数」从写死的 `=== 24` 改成**从内容源数围栏**。
    起因是本轮加两个代码块后这条断言立刻变红 —— 正是记忆里反复强调的
    「别把会变的数写成常量」这个坑，图解数早已改成源里数、代码块数却漏了。
  - 修了一处「术语标记写进 HTML 块」的坑：`llm-04` 的重复惩罚标记最初写进了
    `<p>` 内（remark 不展开 HTML 块里的 `[[id]]`），已改成在 markdown 正文里点题。
- **验证**：`build:fresh` 31 页｜`verify-build` **127/127**｜`wiki-lint` **35/35**｜
  `tasks-facts` 22/22 章达标。代码块 24 → **26**、约 10.5 → 10.6 万汉字，`index.md` / `tasks.md` 已同步。

---

## 2026-09-19 · 第 1 章的样板铺到其余 21 章：名词库扩到 237 条、每章补图解/误区/延伸；新增 `wiki/tasks.md` 进度看板

- **类型**：内容 + 工程
- **改了什么**：
  1. **名词库 25 → 237 条**（`src/data/glossary.ts` 新增 212 条，按 5 个知识域分组）。
     字段按既有契约写：`naming`（译名辨析）、`explain` 带 `<b>`、`meaning` 一句话。
  2. **21 章（第 1 章之外）逐章补五项**：正文用 `[[id]]` 标出 ≥8 个术语；
     图解 1 → **3 张**（原作 1 张 + 每章新写 2 张）；新增「常见误区与追问」小节（≥4 条）；
     新增「参考与延伸」小节（≥3 条外链）；`##` 小节号重排为连续编号。
  3. **题库 72 → 134 问**（每章 +2～3 道，含追问链）。仅在 `src/data/interview.ts` 维护。
  4. **外链台账 54 → 83 条 URL**（`scripts/sources.txt`），`wiki/sources.md` 的总数说法同步。
  5. **新增 `scripts/tasks-facts.mjs` + `wiki/tasks.md`**：逐章完善度看板。
     表格由脚本从正文里数出来，`--write` 写回，**不许手改**（`wiki-lint` 有断言核对）。
     `wiki/tasks.md` 另有一节「已知缺口」人工维护，记下这轮发现但没补的点（见下）。
- **为什么**：原正文太精简 —— 一上来就讲公式与形状，名词（张量 / 投影 / 打分）一个都没解释。
  第 1 章先做成样板（名词卡 + 图解 + 讲解顺序标准），这轮把它铺满全站。
  进度表单独立页，是因为「哪些章补到哪了」既不属于目录（`index.md` 记有什么），
  也不该塞进日志（日志是流水，不是看板）。
- **连带改动**：
  - `scripts/verify-build.mjs` 新增第 12 节：每章「常见误区与追问」≥4 条、「参考与延伸」≥3 条、
    术语卡字段质量（`naming` ≥40 字、`meaning` ≤50 字、`explain` 含 `<b>` 且 ≥100 字）。
  - `scripts/wiki-lint.mjs` 增加 `tasks.md` 的存在与「进度表与实测一致」两条断言。
  - `wiki/index.md`（总量行与每章统计）、`wiki/README.md`（体量说法、文件索引、断言数）、
    `wiki/schema.md`、根 `README.md` 同步。
- **顺手修掉的两处文档缺陷**：
  - `wiki/schema.md` 第十一节有**两组重号的 11.5 / 11.6**（补 11.4.1–11.7 时旧块没删，
    尾部的「断言在哪里」还是缺了新增断言的旧版）。已重排为 11.1–11.8，删掉旧的重号块。
  - 根 `README.md` 的目录树里 `data/` 出现了两次（`glossary.ts` 被单列在 `components/` 之后），已并回。
- **数值修正**（本轮审读时发现前后不一致）：
  - `llm-02` KV cache 显存估算写小了 10 倍（1.05 GiB / 31 GiB → **9.8 GiB / 312 GiB**，32k 上下文）。
  - `harness-06` 信噪比 0.4% → **0.5%**；`harness-08` 的 TTFB 与 inter-chunk 超时写反了，已对调；
    `interview.ts` 的 harness-07 第 ④ 问改为「副作用闸门 + 全量审计」以与正文一致。
- **验证**：`content:check` 0 处问题｜`build:fresh` 31 页｜`verify-build` **127 项**全过｜
  `wiki-lint` **35 项**全过｜`tasks-facts` 22 章全部达标。
  顺手验了新断言真的拦得住：把进度表里一个数字改错，`wiki-lint` 立刻变红。
- **本轮记下但没做的**（已写进 `wiki/tasks.md` 第三节）：`eval-01` 缺显著性检验口径、
  `llm-02` 缺缓存计费、`harness-08` 缺压缩与 prefix 成本权衡、
  几条「画不出来」的抽象概念（All-to-All 拓扑、闭合评测漏斗）、
  词库里 3 条没被任何正文引用的术语（`lenient-parsing` / `prefix-fingerprint` / `repetition-penalty`）、
  以及「重编为 40+ 章」尚未启动。
