...
这个站的正文有 23 章、约 11.5 万汉字、74 张手绘图解、28 段代码、137 道面试问答。
内容分散在四类文件里：
...
└───────────────────────────────────────────────────────────────┘
             ↑ 约束
┌─ 知识层（有什么） ────────────────────────────────────────────┐
│  wiki/index.md       4 领域 / 23 章 / 137 问的完整目录与统计    │
│  wiki/tasks.md       迭代进度看板：逐章完善度 + 待办缺口         │
...
node scripts/tasks-facts.mjs         # 完善度：每章的五项工作各做到哪、缺口是什么
node scripts/tasks-facts.mjs --write # 改完正文后刷新 wiki/tasks.md 的进度表
```
...
| `node scripts/verify-build.mjs` | 产物结构：**127 项**断言（代码块、大翻页区、正文与工具栏同宽、图解动效、侧栏悬停唤出、名词卡片的双语原名 / 命名辨析 / 掌握按钮、每章的「常见误区与追问」「参考与延伸」小节、术语卡片的字段质量、中文标点不许吃掉加粗…） | 每次构建后 |
| `node scripts/wiki-lint.mjs` | 本目录的统计与实际内容是否一致（**35 项**，含 `tasks.md` 进度表不许手改） | 改完正文/题库后 |
| `node scripts/check-links.mjs` | 站外链接是否还活着 | 上线前后 |
| `node tools/verify.mjs` | 活页面交互（**85 项**，需起 preview；含真键盘事件的翻页、真鼠标事件的侧栏唤出、名词卡片弹窗的开关与焦点往返、术语掌握标记的写入 / 取消 / 筛选 / 刷新读回、以及「图解文字有没有被画布裁掉」的全站扫描） | 动过 JS/交互后 |
...
>
> ⚠️ **`shoot.mjs` 的镜头之间会串味**：侧栏收起状态存在 `localStorage`，
> 所以「收起目录」那个镜头之后的每一张图都会少一条侧栏 —— 不报错、不空白，
> 只是默默少一块，而看图的人无法察觉。主循环现在会在每个镜头跳转前统一复位该状态，
> 要收起的镜头请用 `nav: 'collapsed'` **显式声明**，别指望顺序。
> （同类问题：任何时候往 `shoot.mjs` 末尾追加镜头，先想一想它继承了前面哪个镜头留下的状态。）
...
8. **每章必须有 1 张主图 + ≥1 段可运行代码 + 3–4 道问答**（当前 23 章全部满足）。
...
| `wiki/index.md` | 知识目录：4 领域 / 23 章 / 137 问，每章一行 |
| `wiki/tasks.md` | 迭代进度看板：逐章完善度（自动区）+ 已知缺口（人工维护） |
...
| `scripts/wiki-facts.mjs` | 清点脚本：产出 `index.md` 里的统计（章节体量） |
| `scripts/tasks-facts.mjs` | 清点脚本：产出 `tasks.md` 的进度表（逐章完善度） |
