...
- `<defs>` 的位置：**新图统一写在 `<svg>` 开标签之后**（紧跟一行）。
  ⚠️ 这里要如实说一句：站内目前两种情况都有 —— 第 1 章那 7 张（2026-09 新写）是**前置**，
  其余 7 张用到箭头的老图是**后置**。**两者没有任何功能差别**
  （`diagram-motion` 对 `<defs>` 内的内容一律不动，而 SVG 的 marker 引用不受声明位置影响），
  所以不要为了「统一」单独去改那些老图 —— 那是纯 churn。
  另外 28 张图里有 14 张**根本不需要 `<defs>`**（没有箭头连线）。
  新图照着第 1 章写即可。
...
不管名词库里那 25 条** —— 后者在正文里一律写**中文**（`[[forward|前向]]`），
