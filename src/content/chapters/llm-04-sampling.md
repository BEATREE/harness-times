...
<div class="box box-key">
  <span class="box-title">低温更容易复读，重复惩罚是对症的旋钮</span>
  <p>温度越低，模型越「贪」，越容易挑概率最高那批 token，反复说同一段话——Agent 里尤其明显（长任务里模型会把同一句模板车轱辘式重复）。对症的旋钮是<b>重复惩罚</b>：在 logits 阶段对「已经出现过的 token」施加折扣，压住复读冲动。</p>
  <p>但它是旋钮不是开关：<b>过强会误伤本来就该重复的东西</b>——变量名、API 名、专有名词反复出现是正常的，惩罚太重会把它们也压低，输出反而变形。常见起点是 1.1～1.2 左右压复读，一旦发现代码标识符或专名被误伤就调回接近 1.0。它和温度是配合关系：低温本就易复读，适当加一点重复惩罚，往往比单纯再降温度更有效。</p>
</div>

这里的[[repetition-penalty|重复惩罚]]是采样阶段唯一一个「直接动 logits」的旋钮，下一节讲的约束手段都在解码之后、甚至解码之外起作用——记住这个分层，面试时被追问「重复惩罚到底改了什么」才答得准。
