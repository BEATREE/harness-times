...
前面三节都在讲「怎么把记忆写好、写对、写稳」，但记忆系统的另一半——**读取**——经常被漏掉。写进去只是第一步，真正决定效果的是「下一次对话时，能不能在正确的时候把正确的那条捡回来」。这一节补上另一半。

**读取的难点不在「找得到」，而在「该不该放进去」。** 记忆不是越多越好（这也是后面 5.5 误区要讲的）。每次对话开始时，你要从记忆库里挑出**与当前话题相关的、且仍然有效**的那批，塞进上下文——这个「挑」的动作就是读取策略，它有三个关键决策：

1. **按什么召回**：多数系统用「当前消息做 embedding，和记忆做相似度检索」——但这会漏掉「当前消息没提到、但上下文隐含需要」的记忆。更稳的做法是**分层召回**：先用硬规则（实体 id、topic 标签）精确命中，再用语义相似度补充，避免纯向量检索的「看着相关、其实无关」或「精确匹配却漏掉」。
2. **按什么排序**：召回一批后要排序。排序信号至少有三个——**时效**（越新的越优先）、**置信度**（fact 优先于 inference）、**相关性**（与当前意图的匹配度）。不能只按相似度排，否则一条半年前的「用户偏好红色」可能压过昨天刚说的「现在改要蓝色」。
3. **按什么截断**：记忆进上下文会占用预算，必须设上限（比如最多 N 条、或最多 M 字符），并在截断时**优先保留高置信度事实**、舍弃低置信度推断。截断不是「砍掉尾巴」，而是按优先级挑选。

<div class="tbl-wrap">
  <table class="news">
    <caption>读取的三个决策，与每个决策的常见错误</caption>
    <thead><tr><th>决策</th><th>正确做法</th><th>常见错误</th></tr></thead>
    <tbody>
      <tr><td>召回</td><td>硬规则（实体/topic）精确命中 + 语义相似度补充</td><td>只靠纯向量检索，漏召回或误召回</td></tr>
      <tr><td>排序</td><td>时效 + 置信度 + 相关性，三信号加权</td><td>只按相似度排，旧的高相似记忆压过新的</td></tr>
      <tr><td>截断</td><td>设上限，按「高置信事实优先」挑选</td><td>直接砍尾巴，把关键事实留在外面</td></tr>
    </tbody>
  </table>
</div>

**读取与写入的闭环**：读取时若发现「召回了一条已被软删除的旧版本」（`superseded_at` 非空），要自动跳过、只取 `active` 的那条——这就是为什么上一节的软删除不是可有可无的装饰，它直接服务于读取的正确性。同理，读取时若发现两条 `active` 记忆互相矛盾，就触发下一节的冲突消解流程，而不是静默选一条。

一句话：**写入决定「有没有」，读取决定「用不用得对」**。很多记忆系统效果差，不是没存，是读的时候全量塞、无脑排，把预算占满还盖掉了真正有用的那几条。
